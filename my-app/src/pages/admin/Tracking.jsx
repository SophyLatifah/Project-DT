...
export default function Tracking({ orderId }) {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/tracking/${orderId}`);
        if (!res.ok) throw new Error("Gagal fetch data order");
        const data = await res.json();
        console.log("Order dari API:", data);
      ...
      <div className="bg-[#FFB701] p-6 rounded-lg mb-5 shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
          ...
          <div className="grid grid-cols-[120px_1fr] gap-y-2">
            ...
            <span>Order Date</span>
            <span className="font-medium">: {new Date(order.tanggal_order).toLocaleString()}</span>
          ...
          <div className="grid grid-cols-[120px_1fr] gap-y-2">
            ...
            <span>Address</span>
            <span className="font-medium">: {order.alamat_pengiriman}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {steps.map((step, index) => (
          <div
            ...
            className={`p-4 rounded-lg text-center font-medium shadow-md 
              ${
                step.toLowerCase() === order.status_order.toLowerCase()
                  ? "bg-[#219EBC] text-white"
                  : "bg-[#8ECAE6] text-black"
              }
            `}
